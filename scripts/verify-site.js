...
const requiredRegressionContracts = [
  ...
  ["offline shell application cache", /const CACHE_NAME = "nschess-shell-v\d+-[^"]+";[\s\S]*?"\.\/assets\/app\.css\?v=review-v\d+-[^"]+"[\s\S]*?"\.\/assets\/app\.js\?v=(?:review-v\d+-[^"]+|play-human-entry-v\d+|play-human-matchmaking-v\d+)"[\s\S]*?isReviewShellAsset/],
