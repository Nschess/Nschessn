# Nschess Google OAuth setup

Nschess uses its existing Supabase Auth client and returns the Google flow only to a reviewed Nschess origin. Provider secrets belong only in the Supabase dashboard; do not add them to Vercel client variables, `.env.e2e`, source code, or browser logs.
...
| Supabase project callback | `https://jfrvoykwowzthlvzwsmi.supabase.co/auth/v1/callback` |
| Local E2E return URL | `http://127.0.0.1:4173/?auth=oauth` |

The provider callback is Supabase's `/auth/v1/callback`, not a route in this repository. `redirectTo` is the final Nschess return URL and must be allowlisted in Supabase.
...
7. Test with a non-production OAuth account. The Nschess button calls `signInWithOAuth({ provider: "google", options: { redirectTo } })`; it cannot complete until this dashboard configuration is saved.
...
- The trigger safely derives a product username from provider metadata (`username`, `preferred_username`, `user_name`, `full_name`, then `name`) and uses a deterministic `player_<id>` fallback. It never exposes provider email to public identity UI.
- Supabase Auth owns identity linking. Nschess does **not** merge users in browser JavaScript based on an email address. If Supabase associates a Google identity with an existing user, the same `auth.users.id` reaches the existing profile. If it intentionally creates a different user ID, it gets a separate profile rather than risking an unsafe merge.
...
1. Confirm the Google button shows a pending state immediately and cannot be double-clicked.
2. Confirm the generated `redirectTo` is exactly one of the configured Nschess origins, ending in `/?auth=oauth`.
3. Complete one manual Google sign-in using a non-production test identity.
