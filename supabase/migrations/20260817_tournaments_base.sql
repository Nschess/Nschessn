...
create table if not exists public.tournaments (
  ...
  season_key text not null default (extract(year from now())::text || '-S' || (floor((extract(month from now()) - 1) / 3)::integer + 1)::text),
...
alter table public.tournaments add column if not exists season_key text not null default (extract(year from now())::text || '-S' || (floor((extract(month from now()) - 1) / 3)::integer + 1)::text);
alter table public.tournament_awards add column if not exists cosmetic_item_id text;
do $$ begin
...
as $$
  select jsonb_build_object(
    ...
    'seasonKey', tournament.season_key,
    ...
    'awards', coalesce((
      ...
      where award.tournament_id = tournament.id
        and (award.user_id = p_viewer or tournament.status = 'completed')
    ), '[]'::jsonb),
...
begin
  ...
  loop
    ...
    insert into public.tournament_awards (tournament_id, user_id, place, coins, xp, achievement, cosmetic_item_id)
    values (event.id, award.user_id, award.place, coin_reward, xp_reward, case award.place when 1 then 'Tournament Champion' when 2 then 'Tournament Finalist' else 'Tournament Podium' end, cosmetic_item)
...
begin
  ...
  -- Scheduled events become live on the first authenticated lifecycle touch.
  for lifecycle in
  ...
  loop
    ...
    where tournament_id = lifecycle.id and not withdrawn;
    if participant_count >= 2 then
      update public.tournaments
      set status = 'running', current_round = 1,
          ends_at = case when format = 'arena' then coalesce(starts_at, now()) + make_interval(mins => duration_minutes) else null end,
          updated_at = now()
...
begin
  ...
  return public.create_tournament(p_title, p_format, p_visibility, p_clock, p_max_players, p_rounds, p_duration_minutes, null);
end;
$$;
...
begin
  ...
  insert into public.tournaments (code, creator_id, title, format, visibility, clock, max_players, rounds, duration_minutes, starts_at, season_key)
  values (
    ...
    requested_start,
    extract(year from coalesce(requested_start, now()))::text || '-S' || (floor((extract(month from coalesce(requested_start, now())) - 1) / 3)::integer + 1)::text
...
begin
  ...
  if p_action = 'start' then
    ...
    if event.starts_at is not null and event.starts_at > now() then
      raise exception 'This tournament is scheduled for %', to_char(event.starts_at at time zone 'UTC', 'YYYY-MM-DD HH24:MI UTC');
    end if;
    select count(*) into player_count from public.tournament_entries where tournament_id = event.id and not withdrawn;
    if player_count < 2 then raise exception 'At least two players are needed'; end if;
    update public.tournaments
    set status = 'running', current_round = 1, starts_at = coalesce(starts_at, now()), ends_at = case when format = 'arena' then coalesce(starts_at, now()) + make_interval(mins => duration_minutes) else null end, updated_at = now()
    where id = event.id returning * into event;
    perform public.pair_tournament_players(event.id);
